...
async function screenshot(page: import('@playwright/test').Page, testInfo: TestInfo, nombre: string) {
...
Before(async ({ page, $testInfo }) => {
  ...
  await page.reload();
  await screenshot(page, $testInfo, 'dashboard-estado-inicial');
});
...
Then('el gasto aparece en la tabla con monto {string}', async ({ gastosPage, $testInfo }, montoEsperado: string) => {
  const monto = await gastosPage.obtenerMontoPrimeraFila();
  expect(monto).toContain(montoEsperado);
  await screenshot(gastosPage.page, $testInfo, 'gasto-registrado-en-tabla');
});

Then('la tabla muestra al menos un gasto con categoría {string}', async ({ gastosPage, $testInfo }, categoria: string) => {
  ...
  await screenshot(gastosPage.page, $testInfo, `tabla-categoria-${categoria.toLowerCase()}`);
});

Then('la tabla no muestra el gasto eliminado', async ({ gastosPage, $testInfo }) => {
  await expect(gastosPage.mensajeVacio).toBeVisible();
  await screenshot(gastosPage.page, $testInfo, 'tabla-vacia-tras-eliminar');
});

Then('el total del período refleja el monto {string}', async ({ gastosPage, $testInfo }, monto: string) => {
  ...
  await screenshot(gastosPage.page, $testInfo, 'panel-totales-del-periodo');
});
